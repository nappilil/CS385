...
# other functions cannot access child function
def listOfRunLengths(s): # parent function can call child function
    ...
    def lORL(s, result, curCount, curVal): # child function
        ...
        if s == "":
            ...
            return lORL(s[1:], result + [curCount], 1, s[0]) # TO DO (my solution is about 6 lines)
        
            
    return lORL(s[1:], [], 1, s[0]) # don't change this line
    # first run begins with s[0] and has length 1 so far
    # accumulator = result = []
    # curCount: stores length of the current chunk (how many zeroes or ones)
    # curVal: stores current value of either 0 or 1
    # base case if given an empty string; return result (the accumulator)
...
def findRunBits(s):
    ...
    average = sum(listOfRunLengths(s))/len(listOfRunLengths(s))
    n = math.ceil(math.log(average, 2))
    return min(n + 1,7) # TO DO 
